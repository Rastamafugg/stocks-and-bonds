...
- `PACK` will create these module files, but they *may not* be executable by default. Run the command `dir -x -e` to verify that the executable `e` flag has been set. If not, you will need to run the `attr` command on your new module files and turn on the executable flag for the user. For example: `attr /dd/cmds/mymodule perm e`
